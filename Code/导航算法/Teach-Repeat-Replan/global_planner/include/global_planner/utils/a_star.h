...
{     
   ...
   double gScore, fScore; //gscore: distance cost to start point.  fscore: distance cost to final point
   GridNodePtr cameFrom;
   std::multimap<double, GridNodePtr>::iterator nodeMapIt;
   uint8_t * occupancy;   //GridNodeMap[i][j][k]->occupancy = & data[i * GLYZ_SIZE + j * GLZ_SIZE + k];
...
class gridPathFinder
{
	...
	public:
		...
		std::vector<Eigen::Vector3i> getPathIndx();

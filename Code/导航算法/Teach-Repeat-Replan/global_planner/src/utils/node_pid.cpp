...
void NodePID::publishMessage(double angleCommand, double speedCommand)
{
  ...
  cmd_vel.twist.linear.x = speedCommand;
  cmd_vel.twist.linear.y = 0.0;
  cmd_vel.twist.linear.z = 0.0;
  cmd_vel.twist.angular.x = 0.0;
  cmd_vel.twist.angular.y = 0.0;
  cmd_vel.twist.angular.z = angleCommand;
  pubMessage.publish(cmd_vel);
}
...
void NodePID::messageCallback(const nav_msgs::Odometry::ConstPtr& msg)
{
  ...
  ROS_WARN("Start Publish Velovity. speedCommand: %d, angleCommand: %d", fmin(maxSpeed,speedCommand), fmin(maxASpeed,angleCommand));
  speedCommand = fmin(maxSpeed,speedCommand);
  angleCommand = fmin(maxASpeed,angleCommand);
}

...
inactive_pointcloud(new PointCloud)
{
    ...
    neighbor_pointcloud_publish = nh.advertise<PointCloud>("neighbor_pointcloud", 10);
...
void SurfelMap::save_map(const std_msgs::StringConstPtr &save_map_input)
{
    ...
    string mesh_path = save_name + "/mapping_mesh_result.ply";
    printf("save mesh model to %s.\n", mesh_path.c_str());
    save_mesh(save_name);
    string pointcloud_path = save_name + "/mapping_pointcloud_result.pcd";
    printf("save pointcloud to %s.\n", pointcloud_path.c_str());
    save_cloud(pointcloud_path);
    printf("save done!\n");
}
...
void SurfelMap::publish_neighbor_pointcloud(ros::Time pub_stamp, int reference_index)
{
    ...
    neighbor_pointcloud_publish.publish(pointcloud);
